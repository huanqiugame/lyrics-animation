...
| anim_config | AnimationConfig | 自定义全局动画配置（用户通过面板编辑） |
| default_anim_config | AnimationConfig | 默认全局动画配置（初始为空，通过 Alt+导入填充） |
...
**优先级链（6 级，硬编码不计）**：字动画组 > 行动画组 > 自定义全局字动画组 > 自定义全局行动画组 > 默认全局字动画组 > 默认全局行动画组

> 硬编码默认（`resolver.js` 的 `HARDCODED_DEFAULTS`）始终作为最终回退，优先级最低。
...
独立 JSON 文件，**不写入 TTML**。分为两层：

1. **自定义配置**（`anim_config`）：用户通过动画面板编辑，通过普通导入/导出操作
2. **默认配置**（`default_anim_config`）：初始为空数组，通过 Alt+导入填充，Alt+导出时合并到导出文件

```
{
  line_anim_groups: AnimationGroup[],   ← 全局行动画组
  word_anim_groups: AnimationGroup[],   ← 全局字动画组
}
```

**默认配置**（通过预设文件加载，位于 `configs/default/` 目录）：
- 当前预设：**`barely-minimum`**（定义于 `configs/default/barely-minimum.js`，对应 JSON 见同目录 `.json` 文件）
- 由 `js/config-loader.js` 管理加载，`types.js` 的 `createDefaultConfig()` 通过 loader 获取当前预设
- **全局字动画组**：包含 2 个组
  1. 出现动画（wordStart ~ lineEnd 期间，优先级高）：opacity 1→1（字在时间窗口内可见）
  2. 基础样式组（-∞ ~ +∞）：fontFamily、fontSize、color、textShadow、textStroke、opacity 0
- **全局行动画组**：包含 2 个组
  1. 锚点定位（-∞ ~ +∞，优先级高）：anchorPosition=left、anchorOffsetX=20
  2. 文字对齐（-∞ ~ +∞）：textAlign=left
- 未来可在 `configs/default/` 添加更多预设（如 `karaoke`、`fade-slide`），通过 `config-loader.js` 切换

**配置导入/导出**（通过工具栏按钮）：
- **导出（普通点击）**：仅导出 `anim_config`（自定义配置）
- **导出（Alt/Option+点击）**：导出合并后的全量配置（默认 + 自定义）
- **导入（普通点击）**：替换 `anim_config`（自定义配置）
- **导入（Alt/Option+点击）**：替换 `default_anim_config`（默认配置）
