...
/**
 * 将 TTML 时间戳字符串解析为毫秒数
 * @param {string} time_span - 如 "00:06.687" 或 "01:30:05.500"
 * @returns {number} 毫秒数（整数）
 * @throws {TypeError} 格式不匹配时抛出
 */
export function parseTimespan(time_span) {
    const matches = TIME_REGEX.exec(time_span);
    if (!matches) {
        throw new TypeError(`时间戳字符串解析失败：${time_span}`);
    }
 ...
 * @param {number} time_ms - 毫秒数
 * @param {object} [options]
 * @param {boolean} [options.ms=true] - 是否包含毫秒部分
 * @returns {string} 如 "00:06.687"
 */
export function msToTimestamp(time_ms, { ms = true } = {}) {
    let t = time_ms;
